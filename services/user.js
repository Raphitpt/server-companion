...
async function createUser(formData) {
    ...
    const verify = await db.query(
      'SELECT * FROM users WHERE edu_mail = ?',
      [formData.edu_mail]
    );
    ...
    if (verify.length > 0) {
      ...
      const result = await db.query(
        ...
        [formData.pname, formData.name, formData.edu_mail, encryptedPassword, "undefined"]
      );
  
      let message = "Une erreur est survenue";
      if (result.affectedRows > 0) {
        const mailConfirm = await mail.sendConfirmationMail("rtiphonet@gmail.com","Raphaël", "123456789");
